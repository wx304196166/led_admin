...
export default {
  data() {
    return {
      ...
      isEdit: false,
      tableData: [],
      model: labelEntity.model,
      reset: Object.assign({}, labelEntity.model),
      column: labelEntity.tableColumn,
      page: {
        current: 1,
        map: { name: '' },
        size: 10,
  ...
  methods: {
    ...
    remove() {
      ...
      labelApi.batchDelete(ids).then(response => {
        if (response.code === 0) {
    ...
    // 初始化列表数据
    initPageData() {
      labelApi.queryPageList(this.page).then(response => {
        this.tableData = []
        if (response.code === 0) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.isEdit) { // 编辑
            ...
            labelApi.create(this.model).then(response => {
              if (response.code === 0) {
